...
# Estilos CSS personalizados para dar color y estilo profesional
st.markdown("""
    <style>
    .main {
...
# Inicializar almacenamiento de cuentas y turnos
        ...
        with st.form("form_registro"):
            ...
            if btn_registro:
                if not reg_correo or not reg_pass or not reg_nombre:
                    ...
                    st.success("¡Cuenta creada con éxito! Selecciona 'Iniciar Sesión' arriba.")
...
# ==================== APLICACIÓN PRINCIPAL (CHEWY) ====================
    ...
    with st.container():
        ...
        if st.button("💾 Guardar Cierre de Turno en Chewy", type="primary"):
            # CÁLCULOS FINALES INTEGRADORES CON ABONOS (CORREGIDOS)
    ...
    if st.session_state.ultimo_ticket is not None:
        ...
        st.write("Haz clic en el botón de abajo para abrir la ventana de impresión real:")

        t = st.session_state.ultimo_ticket
        
        # HTML limpio y autocontenido para abrirse e imprimirse de inmediato
        ticket_html_contenido = f"""
        <html>
            <head>
                <title>Imprimir Ticket Chewy</title>
                <style>
                    body {{
                        font-family: 'Courier New', Courier, monospace;
                        font-size: 13px;
        ...
        # Codificar el HTML en base64 para integrarlo de forma segura en un enlace web estándar
        import base64
        b64_html = base64.b64encode(ticket_html_contenido.encode('utf-8')).decode('utf-8')
        data_url = f"data:text/html;base64,{b64_html}"

        # Mostrar vista previa visual en la app
        st.markdown(f"""
        ...
        <a href="{data_url}" target="_blank" class="print-btn">
